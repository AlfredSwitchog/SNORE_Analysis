...
%% Leo5 Setup: Add paths to Matlab (adjust for your paths)
    ...
    %% Delete Dummy Scans
    deleteDummiesInFolder(nDummies, OutDir); % numVolumes*2.5 TR suggests that 6 files are delted by the scanner

    %% Slice Timing Correction
    filesSliceTiming = cellstr(spm_select('ExtFPList', OutDir, '^u*'));
    ...
    nc_SliceTimeCorr(filesSliceTiming, nslices, TR, order,sliceTimingFile);
    %% Realign and unwarp
    filesRealign = cellstr(spm_select('FPList', OutDir));
